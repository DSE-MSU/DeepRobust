...
parser.add_argument('--method', default='GLTR', type=str, help='name of detection method')
...
parser.add_argument('--synthetic_dataset_name', type=str, default='', help='name of the customized dataset')
...
def main():
    ...
    print(f'Use DATASET {args.method} to detect', flush=True)
    ...
    if args.target_text:
        ...
        if args.synthetic_dataset_name:
            data_path = f'./detection_results/{args.dataset}_{args.synthetic_dataset_name}/{base_model_name}'
        else:
...
=======
            assert len(final_data['original']) >= args.n_samples, "The current dataset does not have enough data, please generate a new dataset or decrease the number of samples"
...
>>>>>>> LLM_local
            ...
            if args.customized_dataset:
                cur_dataset.generate_data_customized(args.key_name, args.synthetic_dataset_name, args.load_hf_dataset)
            else:
        ...
        if args.method == 'detect_gpt':
            ...
            cur_models = model_collector(base_model=base_model, base_tokenizer=base_tokenizer, classification_model=classification_model, classification_tokenizer=classification_tokenizer, device=device, openai_model=args.openai_model, openai_key=args.openai_key)
