...
def perturb_texts_(texts, model_collector, mask_top_p, span_length, pct, ceil_pct=False, buffer_size=1):
    ...
    #         tokens = model_collector.base_tokenizer(texts, return_tensors="pt", padding=True).to(model_collector.base_model.device)
    #         valid_tokens = tokens.input_ids != model_collector.base_tokenizer.pad_token_id
    #         replace_pct = args.pct_words_masked * (args.span_length / (args.span_length + 2 * buffer_size))

    #         # replace replace_pct of input_ids with random tokens
    #         random_mask = torch.rand(tokens.input_ids.shape, device=model_collector.base_model.device) < replace_pct
    #         random_mask &= valid_tokens
    #         random_tokens = torch.randint(0, model_collector.base_tokenizer.vocab_size, (random_mask.sum(),), device=model_collector.base_model.device)
    #         # while any of the random tokens are special tokens, replace them with random non-special tokens
    #         while any(model_collector.base_tokenizer.decode(x) in model_collector.base_tokenizer.all_special_tokens for x in random_tokens):
    #             random_tokens = torch.randint(0, model_collector.base_tokenizer.vocab_size, (random_mask.sum(),), device=model_collector.base_model.device)
    #         tokens.input_ids[random_mask] = random_tokens
    #         perturbed_texts = model_collector.base_tokenizer.batch_decode(tokens.input_ids, skip_special_tokens=True)
    #     else:
    #         masked_texts = [tokenize_and_mask(x, span_length, pct, ceil_pct, buffer_size) for x in texts]
